...
//             ...data.candidat,
//             alternanceSearch: {
//               ...data.candidat.alternanceSearch,
//               dateDebut: data.candidat.alternanceSearch?.dateDebut
//                 ? new Date(data.candidat.alternanceSearch.dateDebut)
//                     .toISOString()
//                     .split("T")[0]
//                 : "",
//               dateFin: data.candidat.alternanceSearch?.dateFin
//                 ? new Date(data.candidat.alternanceSearch.dateFin)
//                     .toISOString()
//                     .split("T")[0]
//                 : "",
//             },
...
//    useEffect(() => {
//   fetchInformations(); // Appelez-la ici pour le chargement initial
// }, [router]);
...
//     // Gérer les champs avec la notation point (ex: alternanceSearch.posteSouhaite)
//     if (name.includes(".")) {
//       const [parentField, subField] = name.split(".");
//       if (parentField === "alternanceSearch") {
//         setFormData((prev) => ({
//           ...prev,
//           alternanceSearch: {
...
//       }
//     }
//     // Gérer les champs avec la notation bracket (ex: alternanceSearch[dateDebut])
//     else if (name.startsWith("alternanceSearch")) {
...
//       setSuccessMessage(
//         `${field === "cv" ? "CV" : "Vidéo"} sélectionné avec succès : ${file.name}`
//       );
//     }
//   };
...
//       });
//     }
//    if (section === "cv" && formData.cv instanceof File) {
//     formDataToSend.append("cv", formData.cv);
//   }
//   if (section === "video" && formData.video instanceof File) {
...
//                       <div>
//                         <p className="text-gray-700">
//                           École : {profile.formation || "Non spécifié"}
//                         </p>
//                         <p className="text-gray-700">
//                           Poste souhaité :{" "}
...
export default function Profile() {
  ...
  // Références pour l'Upload Widget
  const cloudinaryRef = useRef<any>(null);
  const widgetRef = useRef<any>(null);

  // Charger le script Cloudinary et initialiser le widget
  useEffect(() => {
...
script.onload = () => {
  ...
  widgetRef.current = cloudinaryRef.current.createUploadWidget(
    {
      ...
      uploadPreset: "unsigned_upload_candidats",
      sources: ["local", "url", "camera"],
      multiple: false,
      maxFileSize: 500000000,
      folder: "candidats",
      clientAllowedFormats: ["jpg", "png", "mp4", "webm", "pdf"], // Conserver tous les formats
      chunkSize: 6000000,
    },
    async (error: any, result: any) => {
      if (!error && result && result.event === "success") {
        ...
        // Utiliser resource_type et/ou format pour une meilleure distinction
        if (resource_type === "video") {
          newFormData = { ...newFormData, videoUrl: secure_url };
          sectionToUpdate = "video";
        } else if (resource_type === "image" && (format === "jpg" || format === "png")) {
          // C'est une image JPG/PNG
          newFormData = { ...newFormData, photoUrl: secure_url };
          sectionToUpdate = "photo";
        } else if (resource_type === "raw" || format === "pdf") {
          // C'est un PDF (raw ou format 'pdf')
          newFormData = { ...newFormData, cvUrl: secure_url };
          sectionToUpdate = "cv";
        } else {
          // Fallback ou gestion d'erreur si le type n'est pas reconnu
          setError("Type de fichier non reconnu ou mauvaise classification Cloudinary.");
          return;
        }

        setFormData(newFormData); // Mettre à jour l'état formData une seule fois

        if (sectionToUpdate) {
          // Déclenche la fonction de soumission pour mettre à jour le profil en base de données
          // et ensuite déclenche un fetchInformations pour rafraîchir le composant.
          // Le setTimeout est une petite astuce pour s'assurer que setFormData a terminé son travail.
          // Une autre approche pourrait être un useEffect avec formData comme dépendance.
          await handleSubmitWithUrl(sectionToUpdate, secure_url);
        }
      }
      if (error) {
    ...
    const handleSubmitWithUrl = async (section: string, url: string) => {
      setIsLoading((prev) => ({ ...prev, [section]: true }));
      setSuccessMessage(null);
      const token = Cookies.get("token");
      ...
      if (section === "cv") {
        formDataToSend.append("cvUrl", url);
      } else if (section === "video") {
        formDataToSend.append("videoUrl", url);
      } else if (section === "photo") {
        formDataToSend.append("photoUrl", url);
      }
  ...
  const formDataToSend = new FormData();
  let changesDetected = false; // Flag pour vérifier si des changements textuels ont eu lieu

  // Seules les sections textuelles sont gérées ici
  if (section === "personalInfo" || section === "alternanceSearch") {
    allowedFields.forEach((field) => {
      // Vérifier si le champ est textuel et s'il a changé
      if (
        (field === "firstName" || field === "lastName" || field === "email" || field === "phone" || field === "location" || field === "formation") || // Champs directs
        (field === "alternanceSearch" && typeof formData[field] === "object") // Champ objet
      ) {
        const currentValue = profile?.[field as keyof ICandidat]; // Valeur actuelle du profil
        const newValue = formData[field as keyof ICandidat]; // Nouvelle valeur du formData

        if (field === "alternanceSearch" && typeof newValue === "object") {
            // Comparaison plus profonde pour alternanceSearch
            const currentAlternanceSearch = profile?.alternanceSearch || {};
            const newAlternanceSearch = newValue as typeof profile.alternanceSearch;
            
            // Convertir les dates en chaînes pour comparaison si elles sont des objets Date
            const currentDatesAsString = {
                dateDebut: currentAlternanceSearch.dateDebut instanceof Date ? currentAlternanceSearch.dateDebut.toISOString().split('T')[0] : currentAlternanceSearch.dateDebut,
                dateFin: currentAlternanceSearch.dateFin instanceof Date ? currentAlternanceSearch.dateFin.toISOString().split('T')[0] : currentAlternanceSearch.dateFin,
            };
            const newDatesAsString = {
                dateDebut: newAlternanceSearch?.dateDebut instanceof Date ? newAlternanceSearch.dateDebut.toISOString().split('T')[0] : newAlternanceSearch?.dateDebut,
                dateFin: newAlternanceSearch?.dateFin instanceof Date ? newAlternanceSearch.dateFin.toISOString().split('T')[0] : newAlternanceSearch?.dateFin,
            };

            const alternanceSearchChanged = Object.keys(newAlternanceSearch || {}).some(key => {
                const subField = key as keyof typeof newAlternanceSearch;
                if (subField === "dateDebut" || subField === "dateFin") {
                    // Comparer les dates comme des chaînes
                    return currentDatesAsString[subField] !== newDatesAsString[subField];
                }
                return currentAlternanceSearch[subField] !== newAlternanceSearch[subField];
            });

            if (alternanceSearchChanged) {
                formDataToSend.append(field, JSON.stringify(newValue));
                changesDetected = true;
            }
        } else if (String(currentValue) !== String(newValue) && newValue !== undefined && newValue !== null) {
          // Comparaison pour les champs simples
          formDataToSend.append(field, String(newValue));
          changesDetected = true;
        }
      }
    });
  } else {
    // Si une section non textuelle est appelée ici, c'est une erreur de logique
    console.warn("handleSubmit appelé avec une section de fichier. Ceci ne devrait pas arriver.");
    setIsLoading((prev) => ({ ...prev, [section]: false }));
    return;
  }
...
// ... et la fonction handleCancel doit également réinitialiser profile, pas formData
const handleCancel = (section: string) => {
    // Réinitialiser formData avec les données du profile
    if (profile) {
        setFormData({
            ...profile,
            // S'assurer que les dates sont formatées pour l'input date si elles existent
            alternanceSearch: {
                ...profile.alternanceSearch,
                dateDebut: profile.alternanceSearch?.dateDebut
                    ? new Date(profile.alternanceSearch.dateDebut).toISOString().split("T")[0]
                    : "",
                dateFin: profile.alternanceSearch?.dateFin
                    ? new Date(profile.alternanceSearch.dateFin).toISOString().split("T")[0]
                    : "",
            },
        });
    }
                  ...
                  <div className="flex items-center">
                    <div className="relative w-12 sm:w-16 h-12 sm:h-16 bg-[#7A20DA] text-white flex items-center justify-center rounded-full mr-4 cursor-pointer group">
                      ...
                      <img
                        ...
                        src={
                          formData.photoUrl || profile.photoUrl || "/avatar.png"
                        }
                        className="absolute inset-0 w-full h-full object-cover rounded-full"
                      />
                      ...
                      <button
                        className="absolute bottom-0 right-0 bg-white text-[#7A20DA] rounded-full p-1 cursor-pointer"
                        onClick={() => {
                          setIsEditing((prev) => ({ ...prev, photo: true }));
            ...
            {activeTab === "profile" && (
              <div className="bg-white rounded-[20px] shadow-lg p-4 sm:p-6 md:p-8 mt-4 sm:mt-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                  ...
                  <div className="border border-[#C4C4C4] p-4 rounded-lg h-[400px] sm:h-[600px]">
                    <iframe
                      key={profile.cvUrl || "no-cv"}
                    src={`https://docs.google.com/gview?url=${encodeURIComponent(
      formData.cvUrl || profile.cvUrl || ""
    )}&embedded=true`}
                      className="w-full h-full"
                      title="CV du candidat"
                    />
                      ...
                      <div className="flex space-x-2 mt-2 sm:mt-0">
                        ...
                        {!isEditing.cv && (
                          <button
                            className="border-[#7A20DA] cursor-pointer border bg-white text-[#7A20DA] px-6 sm:px-8 py-2 rounded-[5px]"
                            onClick={() => {
                              setIsEditing((prev) => ({ ...prev, cv: true }));
                              widgetRef.current.open();
                            }}
                          >
                            Modifier
                        ...
                        {isEditing.cv && (
                          <div className="sm:flex space-y-2 sm:space-y-0 sm:space-x-2">
                            ...
                            <button
                              className="border-[#7A20DA] border cursor-pointer bg-white text-[#7A20DA] px-4 sm:px-8 py-2 mr-2 rounded-[5px]"
                              onClick={() => handleSubmit("cv")}
                              disabled={isLoading.cv || !formData.cvUrl}
                            >
                              {isLoading.cv ? "Chargement..." : "Sauvegarder"}
                            </button>
                            <button
                        ...
                        {isEditing.video && (
                          <div className="sm:flex space-y-2 sm:space-y-0 sm:space-x-2">
                            ...
                            <button
                              className="border-[#7A20DA] border cursor-pointer bg-white text-[#7A20DA] px-4 sm:px-8 py-2 mr-2 rounded-[5px]"
                              onClick={() => handleSubmit("video")}
                              disabled={isLoading.video || !formData.videoUrl}
                            >
                              {isLoading.video
                                ? "Chargement..."
                                : "Sauvegarder"}
