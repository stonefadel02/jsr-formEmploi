...
export default function Profile() {
  ...
  // États pour l'édition des champs textes
  const [isEditing, setIsEditing] = useState<{ [key: string]: boolean }>({
    personalInfo: false,
    alternanceSearch: false,
    // Note: cv, video, photo ne sont plus vraiment des modes "édition" mais des actions d'upload
    cv: false, 
    video: false,
    photo: false,
  });
  ...
  const [isLoading, setIsLoading] = useState<{ [key: string]: boolean }>({
    ...
    alternanceSearch: false,
    cv: false, // Upload CV
    video: false, // Upload Video
    photo: false, // Upload Photo
    downloadCv: false,
    downloadVideo: false,
  });
  ...
  // --- NOUVEAU SYSTÈME D'UPLOAD DIGITAL OCEAN ---
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [uploadType, setUploadType] = useState<"cv" | "video" | "photo" | null>(null);

  // 1. Fonction pour déclencher l'input fichier caché
  const triggerFileUpload = (type: "cv" | "video" | "photo") => {
    setUploadType(type);
    setError(null);
    setSuccessMessage(null);
    // Petit délai pour s'assurer que le state uploadType est à jour
    setTimeout(() => {
       fileInputRef.current?.click();
    }, 100);
  };

  // 2. Fonction qui sauvegarde l'URL en base de données après l'upload S3
  const saveUrlToBackend = async (type: "cv" | "video" | "photo", url: string) => {
    ...
    if (type === "cv") formDataToSend.append("cvUrl", url);
    if (type === "video") formDataToSend.append("videoUrl", url);
    if (type === "photo") formDataToSend.append("photoUrl", url);
    ...
    try {
        ...
        if (res.ok) {
            await fetchInformations(); // Rafraîchir les données
            setSuccessMessage(
                type === "cv" ? "CV mis à jour !" : 
                type === "video" ? "Vidéo mise à jour !" : "Photo mise à jour !"
            );
            setTimeout(() => setSuccessMessage(null), 3000);
        } else {
            const data = await res.json();
            setError(data.error || "Erreur lors de la sauvegarde en base de données.");
        }
    } catch (err) {
        console.error(err);
  ...
  // 3. Fonction principale d'upload (S3 -> Backend)
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file || !uploadType) return;

    // Activer le loading spécifique
    setIsLoading(prev => ({ ...prev, [uploadType]: true }));
    setError(null);

    try {
        // A. Obtenir l'URL pré-signée
        ...
        if (!res.ok) throw new Error("Erreur lors de la préparation de l'upload.");
        const { uploadUrl, publicUrl } = await res.json();

        // B. Uploader le fichier vers DigitalOcean
        ...
        if (!uploadRes.ok) throw new Error("Échec du téléversement vers le serveur.");

        // C. Sauvegarder l'URL finale dans le backend
        await saveUrlToBackend(uploadType, publicUrl);

    } catch (err: any) {
        console.error(err);
        setError(err.message || "Une erreur est survenue lors de l'upload.");
        setIsLoading(prev => ({ ...prev, [uploadType]: false }));
    } finally {
        // Reset de l'input
  ...
  const handleChange = (
    ...
    // Gestion des champs imbriqués (alternanceSearch)
    if (name.includes(".")) {
  ...
  const allowedFields: (keyof ICandidat)[] = [
    "firstName", "lastName", "email", "phone", "location", "alternanceSearch"
  ];

  // Sauvegarde des champs TEXTES (Infos perso, Alternance)
  const handleSubmit = async (section: string) => {
    ...
    if (section === "personalInfo" || section === "alternanceSearch") {
        allowedFields.forEach((field) => {
            // Logique de détection de changement (identique à votre code original)
            const currentValue = profile?.[field as keyof ICandidat];
            const newValue = formData[field as keyof ICandidat];
            ...
            if (field === "alternanceSearch" && typeof newValue === "object") {
                ...
                const changed = Object.keys(newAlternanceSearch || {}).some(key => {
                    const k = key as keyof typeof newAlternanceSearch;
                    if (k === "dateDebut" || k === "dateFin") return currentDates[k] !== newDates[k];
                    return currentAlternanceSearch[k] !== newAlternanceSearch[k];
                });
...
const handleDownloadCv = async () => {
    if (!profile?.cvUrl) return alert("Aucun CV disponible.");
    setIsLoading((prev) => ({ ...prev, downloadCv: true }));
    try {
      // CORRECTION : On retire le header Authorization car le fichier est public sur DO
      ...
      link.href = url;
      // On donne un nom propre au fichier téléchargé
      link.setAttribute("download", `CV_${profile.firstName || 'Candidat'}_${profile.lastName || ''}.pdf`);
      ...
      console.error(error);
      // Fallback : Si le téléchargement blob échoue (ex: CORS strict), on ouvre dans un nouvel onglet
  ...
  const handleDownloadVideo = async () => {
    if (!profile?.videoUrl) return alert("Aucune vidéo disponible.");
    setIsLoading((prev) => ({ ...prev, downloadVideo: true }));
    try {
      // CORRECTION : On retire le header Authorization
      ...
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `Video_${profile.firstName || 'Candidat'}_${profile.lastName || ''}.mp4`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
      ...
      console.error(error);
      // Fallback
      window.open(profile.videoUrl, '_blank');
    } finally {
      setIsLoading((prev) => ({ ...prev, downloadVideo: false }));
    }
  };

  if (error && !profile) { // Afficher erreur seulement si pas de profil chargé
    return (
      <>
        <Navbar />
        <div className="min-h-screen bg-gradient-to-l from-[#8E2DE2] to-[#4B00C8] flex items-center justify-center px-4 py-8">
  ...
  if (!profile) {
    return (
      <>
        <Navbar />
        <div className="min-h-screen bg-gradient-to-l from-[#7A20DA] to-[#4B00C8] flex items-center justify-center px-4 py-8">
