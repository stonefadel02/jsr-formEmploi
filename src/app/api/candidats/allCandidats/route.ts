...
interface QueryParams {
  ...
  location?: string;
  level?: string;
  contracttype?: string; // Ajout du filtre contracttype
...
interface MatchQuery {
  ...
  'alternanceSearch.contracttype'?: string; // Ajout du champ contracttype
}
...
export async function GET(req: NextRequest): Promise<NextResponse<ApiResponse<{ candidats: ICandidat[]; total: number }>>> {
  try {
    ...
    const query: QueryParams = {
      ...
      contracttype: searchParams.get('contracttype') || undefined, // Ajout du paramètre
    ...
    if (query.contracttype) matchQuery['alternanceSearch.contracttype'] = query.contracttype; // Ajout du filtre
    ...
    switch (query.sortBy) {
      case 'date':
        ...
        break;
      case 'relevance':
        sortOption['personalInfo.firstName'] = 1;
        break;
      case 'location':
