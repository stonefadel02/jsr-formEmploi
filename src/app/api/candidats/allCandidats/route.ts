...
import { connectEmployersDb } from "@/lib/mongodb"; // Assurez-vous d'importer les deux
...
export async function GET(
  ...
  NextResponse<ApiResponse<{ candidats: ICandidat[]; total: number }>>
> {
  try {
    ...
    if (
      !employer ||
      (employer.role !== "employeur" && employer.role !== "admin")
    ) {
      return NextResponse.json(
        { success: false, message: "Employeur access required" },
        { status: 403 }
      );
    }
    ...
    const query: QueryParams = {
      ...
      sortBy:
        (searchParams.get("sortBy") as "date" | "relevance" | "location") ||
        "date",
    ...
    if (query.location)
      matchQuery["alternanceSearch.location"] = query.location;
    if (query.level) matchQuery["alternanceSearch.level"] = query.level;
    if (query.contracttype)
      matchQuery["alternanceSearch.contracttype"] = query.contracttype;
    ...
    // On ajoute le reste du pipeline
